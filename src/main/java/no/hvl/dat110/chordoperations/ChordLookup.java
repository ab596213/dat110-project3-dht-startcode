...
public class ChordLookup {
	...
	public NodeInterface findSuccessor(BigInteger key) throws RemoteException {
		...
		// get the successor of the nodeid
		NodeInterface temp = node.getSuccessor();
		...
		if (Util.checkInterval(key, node.getNodeID().add(en), temp.getNodeID().subtract(en))) {
			// if logic returns true, then return the successor
			return temp;
		} else {
			// if logic returns false; call findHighestPredecessor(key)
			NodeInterface rekursiv = findHighestPredecessor(key);
			// do highest_pred.findSuccessor(key) - This is a recursive call until logic returns true
			return rekursiv.findSuccessor(key);
		}
	...
	private NodeInterface findHighestPredecessor(BigInteger ID) throws RemoteException {
		...
		for (int i = list.size()-1; i >= 0; i--) {
			...
			NodeInterface tempd = Util.getProcessStub(list.get(i).getNodeName(), list.get(i).getPort());
			// check that finger is a member of the set {nodeID+1,...,ID-1} i.e. (nodeID+1 <= finger <= key-1) using the ComputeLogic
			BigInteger en = BigInteger.ONE;
			if (Util.checkInterval(tempd.getNodeID(), node.getNodeID().add(en), ID.subtract(en))) {
